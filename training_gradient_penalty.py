...
if __name__ == "__main__":
    ...
    optimizer_G = optim.Adam(fsg.parameters(), lr=learning_rate, betas=(b1, b2))
    optimizer_D = optim.Adam(vdis.parameters(), lr=learning_rate, betas=(b1, b2))
    ...
    for epoch in epoch_bar:
        ...
        for iter, real_video in tqdm(enumerate(dataloader), total=len(dataloader), leave=False):
            ...
            real_video = real_video.view(-1, *real_video.shape[-3:])
            #######################
            # Train discriminator #
            #######################
            ...
            for _ in range(iterD):
                ...
                _, real_latent = progan.netD(real_video, getFeature=True)
                ...
                alpha = torch.randn((real_video.shape[0], 1, 1, 1), device='cuda', requires_grad=True)
                interpolation = alpha * real_video + (1 - alpha) * fake_video

                _, gp_latent = progan.netD(interpolation, getFeature=True)  # (bs * N, 512)
                gp_latent = gp_latent.view(batch_size, n_frames, -1)  # (bs, N, 512)
                interp_logits = vdis(gp_latent)  # (bs, 1)
                ...
                gradient_penalty = torch.mean(torch.square(gradients.norm(2, dim=1) - 1))

                # Update discriminator weights
                loss_D = real_loss + fake_loss + 0.2 * gradient_penalty
                loss_D.backward()
                optimizer_D.step()
            ...
            gen_similarity += loss_similarity.item() if alpha_similarity > 0 else 0
            total += batch_size
            ...
            if step % iter_logger == 0:
                ...
                log_writer.add_scalar('Generator loss/similarity', gen_similarity / iter_logger, step)
